...
public class HandleService : EasyBot
{
    ...
    protected override async Task OnPrivateChat(Chat chat, User user, UpdateInfo update)
    {
	    ...
	    if (update.UpdateKind != UpdateKind.NewMessage || update.MsgCategory != MsgCategory.Text)
		    return;
	    UserField:
	    var userButtons = new KeyboardButton[][]
	    {
		    new KeyboardButton[] { "Javoblarni yuborish" }
	    };
	    await Telegram.SendTextMessageAsync(chat.Id, "Hush kelibsiz!",
		    replyMarkup: new ReplyKeyboardMarkup(userButtons) { ResizeKeyboard = true });
	    var userResponse = await NewTextMessage(update);
	    //shu yerdan davom etib ketsez boladi user funksiyalari uchun
